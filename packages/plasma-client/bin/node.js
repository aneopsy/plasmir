...
const PlasmaCore = require('@plasmir/plasma-core')
...
program
  ...
  .option(
    '-r, --registry <address>',
    'Plasma chain registry address',
    '0x18d8BD44a01fb8D5f295a2B3Ab15789F26385df7'
  )
  .option(
    '-c, --chain <name>',
    'Name of the plasma chain to connect to.',
    'PG-beta.14'
  )
  .option(
    '-e, --ethereum <endpoint>',
    'Endpoint of the Ethereum node to connect to.',
    'https://rinkeby.infura.io/v3/fce31f1fb2d54caa9b31ed7d28437fa5'
  )
  .option(
    '-h, --hostname <hostname>',
    'Host to run the client on.',
    'localhost'
  )
  .option('-p, --port <port>', 'Port to run the client on.', '9898')
  .option(
    '-w, --wallet <mode>',
    'Which wallet mode to use. Either "remote" or "local".',
    'local'
  )
  .option('-d, --debug', 'Runs the node in debug mode.')
  .option(
    '-f, --finality <blocks>',
    'Number of blocks to wait before considering events final.',
    parseInt,
    0
  )
  .parse(process.argv)
...
;(async () => {
  ...
  if (pkg.version < latest) {
    ...
    console.log()
    console.log(
      `You might also want to reset your database (this won't delete your accounts):`
    )
  ...
  console.log(
    `Please do NOT use this application with real money (unless you're willing to lose it).`
  )
