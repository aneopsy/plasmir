...
const Transaction = require('@plasmir/plasma-utils').serialization.models
  .Transaction
...
// const encoder = require('@plasmir/plasma-utils').encoder
...
async function setupPlasma() {
  ...
  const serCt = new web3.eth.Contract(serAbi, addr, {
    from: addr,
    gas: 7000000,
    gasPrice: '3000',
  })
  ...
  const plasmaCt = new web3.eth.Contract(abi, addr, {
    from: addr,
    gas: 7000000,
    gasPrice: '3000',
  })
  ...
  debugger
  operatorSetup = await plasma.methods
    .setup(web3.eth.accounts.wallet[0].address, weiDecimalOffset, ser._address)
    .send()
...
async function setupToken() {
  ...
  const tokenCt = new web3.eth.Contract(tokenAbi, addr, {
    from: addr,
    gas: 6500000,
    gasPrice: '3000',
  })
  ...
  token = await tokenCt
    .deploy({
      arguments: [name, ticker, decimals, totSupply],
      data: tokenBytecode,
    })
...
function asBytes32(string) {
  ...
  let pad = []
  for (let i = 0; i < remainingNumBytes; i++) {
    pad.push(0)
  }
