...
const plasmaUtils = require('@plasmir/plasma-utils')
...
describe('Plasma Smart Contract', () => {
  ...
  describe('Operator Usage', () => {
    it('should allow a block to be published by the operator', async () => {
      await plasma.methods
        .submitBlock(dummyBlockHash)
        .send({
          value: 0,
  ...
  describe('Serialization Decoding', () => {
    ...
    before(async () => {
      ...
      randomTransferEncoding =
        '0x' + new Transfer(randomTX.transfers[randomTransferIndex]).encoded
      encodedSignature =
      ...
      decodedTransactionProof = {
        transferProofs: [decodedTransferProof],
      }
    ...
    describe('Transfer Decoding', () => {
      it('should decode a transfer sender', async () => {
        const decoded = await serializer.methods
          .decodeSender(randomTransferEncoding)
          .call()
        const expected = randomTX.transfers[
          randomTransferIndex
        ].sender.toLowerCase()
      ...
      it('should decode a transfer recipient', async () => {
        const decoded = await serializer.methods
          .decodeRecipient(randomTransferEncoding)
          .call()
        const expected = randomTX.transfers[
          randomTransferIndex
      ...
      it('should decode a token type bytes', async () => {
        const decoded = await serializer.methods
          .decodeTokenTypeBytes(randomTransferEncoding)
          .call()
        const expected =
          '0x' + randomTX.transfers[randomTransferIndex].token.toString(16, 8)
        assert.equal(decoded, expected)
      })
      it('should decode a token type as uint', async () => {
        const decoded = await serializer.methods
          .decodeTokenType(randomTransferEncoding)
          .call()
        const expected = randomTX.transfers[
          randomTransferIndex
        ].token.toString()
      ...
      it('should convert an untyped coinID to typed given the tokenType', async () => {
        const untyped = 5
        const type = 2
        const typed = await serializer.methods
          .getTypedFromTokenAndUntyped(type, untyped)
          .call()
      ...
      it('should decode a transfer range', async () => {
        const decoded = await serializer.methods
          .decodeTypedTransferRange(randomTransferEncoding)
          .call()
        const expectedType = randomTX.transfers[
          randomTransferIndex
        ].token.toString(16, 8)
        const expectedStart = randomTX.transfers[
          randomTransferIndex
        ].start.toString(16, 12)
        const expectedEnd = randomTX.transfers[
          randomTransferIndex
        ].end.toString(16, 12)
        const expected = [
    ...
    describe('Transaction Decoding', () => {
      it('should getLeafHash of an encoded transaction', async () => {
        const possibleHash = await serializer.methods
          .getLeafHash(randomTXEncoding)
          .call()
        assert.equal(possibleHash, randomTX.hash)
      })
      it('should decodeBlockNumber from a tx', async () => {
        const decoded = await serializer.methods
          .decodeBlockNumber(randomTXEncoding)
          .call()
      ...
      it('should decodeNumTransfers from a tx', async () => {
        const decoded = await serializer.methods
          .decodeNumTransfers(randomTXEncoding)
          .call()
      ...
      it('should decode the ith transfer', async () => {
        const index = 0
        const decoded = await serializer.methods
          .decodeIthTransfer(index, randomTXEncoding)
          .call()
        const transfer = randomTX.transfers[index]
        const expected =
          '0x' + new Transfer(transfer).encoded.toString().toLowerCase()
        assert.equal(decoded, expected)
      })
    })
    describe('Transfer Proof Decoding', () => {
      it('should decodeParsedSumBytes', async () => {
        const decoded = await serializer.methods
          .decodeParsedSumBytes('0x' + encodedTransferProof)
          .call()
      ...
      it('should decodeLeafIndex', async () => {
        const decoded = await serializer.methods
          .decodeLeafIndex('0x' + encodedTransferProof)
          .call()
      ...
      it('should decodeSignature', async () => {
        const decoded = await serializer.methods
          .decodeSignature('0x' + encodedTransferProof)
          .call()
        const expected = [
      ...
      it('should decodeNumInclusionProofNodesFromTRProof', async () => {
        const decoded = await serializer.methods
          .decodeNumInclusionProofNodesFromTRProof('0x' + encodedTransferProof)
          .call()
      ...
      it('should decodeIthInclusionProofNode', async () => {
        const decoded = await serializer.methods
          .decodeIthInclusionProofNode(0, '0x' + encodedTransferProof)
          .call()
        const expected =
          '0x' + new BigNum(testTransferProof.inclusionProof[0]).toString(16)
        assert.equal(decoded, expected)
      })
    })
    describe('Transaction Proof Decoding', () => {
      it('should decodeNumTransactionProofs', async () => {
        const decoded = await serializer.methods
          .decodeNumTransactionProofs('0x' + encodedTransactionProof)
          .call()
        const expected = new BigNum(
          transactionProof.transferProofs.length
        ).toString()
        assert.equal(decoded, expected)
      })
      ...
      it('should decodeNumInclusionProofNodesFromTXProof', async () => {
        const decoded = await serializer.methods
          .decodeNumInclusionProofNodesFromTXProof(
            '0x' + encodedTransactionProof
          )
          .call()
        const expected = testTransferProof.inclusionProof.length
        assert.equal(decoded, expected)
  ...
  describe('Proof Checking', () => {
    ...
    it('should checkTransferProofAndGetTypedBounds', async () => {
      ...
      const possibleImplicitBounds = await plasma.methods
        .checkTransferProofAndGetTypedBounds(
      ...
      assert(
        new BigNum(possibleImplicitBounds[1]).gte(
          new BigNum(txs[0].transfers[0].end)
        )
      )
    })
    it('should checkTransactionProofAndGetTypedTransfer', async () => {
      const unsigned = new UnsignedTransaction(tx)
      const requestedTransfer = await plasma.methods
  ...
  describe('Deposits and Exits', () => {
    it('should allow a first deposit and add it to the deposits correctly', async () => {
      ...
      const exitableStart = await plasma.methods
        .exitable__untypedStart(0, depositSize)
        .call()
      const depositStart = await plasma.methods
        .deposits__untypedStart(0, depositSize)
        .call()
      const depositer = await plasma.methods
        .deposits__depositer(0, depositSize)
        .call()
    ...
    it('should allow a second deposit and add it to the deposits correctly', async () => {
      ...
      await plasma.methods
        ...
        .send({
          value: depositSize,
          from: web3.eth.accounts.wallet[2].address,
          gas: 4000000,
        })
      const depositEnd = 550 // 550 hardcoded from above deposit of 50
      const exitableStart = await plasma.methods
        .exitable__untypedStart(0, depositEnd)
        .call()
      const depositStart = await plasma.methods
        .deposits__untypedStart(0, depositEnd)
        .call()
      const depositer = await plasma.methods
        .deposits__depositer(0, depositEnd)
        .call()
      assert.equal(exitableStart, '0')
    ...
    it('should allow that users `beginExit`s', async () => {
      ...
      const exitEnd = '10'
      await plasma.methods
        .beginExit(0, plasmaBlock, exitStart, exitEnd)
        .send({
          value: 0,
          from: web3.eth.accounts.wallet[1].address,
          gas: 4000000,
    ...
    it('should properly finalize leftmost, rightmost, and middle exits', async () => {
      ...
      const plasmaBlock = '0'
      await plasma.methods
        .beginExit(0, plasmaBlock, 300, 550)
        .send({
          value: 0,
          from: web3.eth.accounts.wallet[1].address,
          gas: 4000000,
      ...
      await plasma.methods
        .beginExit(0, plasmaBlock, 100, 200)
        .send({
          value: 0,
          from: web3.eth.accounts.wallet[1].address,
          gas: 4000000,
      ...
      const firstExitableStart = await plasma.methods
        .exitable__untypedStart(0, 100)
        .call()
      const secondExitableStart = await plasma.methods
        .exitable__untypedStart(0, 300)
        .call()
  ...
  describe('Exit games', () => {
    ...
    const txA = new SignedTransaction({
      ...
      ...{
        signatures: [
          plasmaUtils.utils.sign(
            txAUnsigned.hash,
            web3.eth.accounts.wallet[1].privateKey
          ),
        ],
      },
    })
    ...
    const txB = new SignedTransaction({
      ...
      ...{
        signatures: [
          plasmaUtils.utils.sign(
            txBUnsigned.hash,
            web3.eth.accounts.wallet[2].privateKey
          ),
        ],
      },
    })
    ...
    const txC = new SignedTransaction({
      ...
      ...{
        signatures: [
          plasmaUtils.utils.sign(
            txCUnsigned.hash,
            web3.eth.accounts.wallet[3].privateKey
          ),
        ],
      },
    })
    ...
    before(async function() {
      ...
      await plasma.methods
        .submitBlock(dummyBlockHash)
        .send({ value: 0, from: operator, gas: 4000000 })
        .catch((error) => {
          console.log('send callback failed: ', error)
        })
      await plasma.methods
      ...
      await plasma.methods
        .depositETH()
        .send({ value: end, from: alice, gas: 4000000 })

      await plasma.methods
        .submitBlock('0x' + blocks['A'].root().hash)
        .send({ value: 0, from: operator, gas: 4000000 })
      await plasma.methods
        .submitBlock('0x' + blocks['B'].root().hash)
        .send({ value: 0, from: operator, gas: 4000000 })
      await plasma.methods
        .submitBlock('0x' + blocks['C'].root().hash)
        .send({ value: 0, from: operator, gas: 4000000 })
    })
    it('should allow inclusionChallenges and respondTransactionInclusion', async () => {
      await plasma.methods
        .beginExit(0, 5, start, end)
        .send({ value: 0, from: dave, gas: 4000000 })
      let exitID = exitNonce // this will be 0 since it's the first exit
      exitNonce++

      await plasma.methods
        .challengeInclusion(exitID)
      ...
      const chalCount = await plasma.methods
        .exits__challengeCount(exitID)
        .call()
      ...
      const newChalCount = await plasma.methods
        .exits__challengeCount(exitID)
        .call()
      assert.equal(newChalCount, '0')

      const isOngoing = await plasma.methods
        .inclusionChallenges__ongoing(chalID)
        .call()
      assert.equal(isOngoing, false)
    })
    it('should allow respondDepositInclusion s', async () => {
      await plasma.methods
        .beginExit(0, 2, start, end)
        .send({ value: 0, from: alice, gas: 4000000 })
      ...
      await plasma.methods.respondDepositInclusion(chalID, end).send()
    ...
    it('should allow Spent Coin Challenges to cancel exits', async () => {
      ...
      await plasma.methods
        .beginExit(0, 3, start, end)
        .send({ value: 0, from: carol, gas: 4000000 })
    ...
    it('should allow challengeBeforeDeposit s', async () => {
      await plasma.methods
        .beginExit(0, 0, start, end)
        .send({ value: 0, from: alice, gas: 4000000 })
      const exitID = exitNonce
      exitNonce++

      const coinID = 0 // anything in the deposit, 0-99
      await plasma.methods.challengeBeforeDeposit(exitID, coinID, end).send()
    ...
    it('should allow challengeInvalidHistoryWithTransaction s and respondInvalidHistoryTransaction s', async () => {
      ...
      await plasma.methods
        .challengeInvalidHistoryWithTransaction(
          exitID,
          coinID,
          transferIndex,
      ...
      await plasma.methods
        .respondInvalidHistoryTransaction(
          chalID,
          transferIndex,
    ...
    it('should allow respondInvalidHistoryDeposit s', async () => {
      ...
      await plasma.methods
        .depositETH()
        .send({ value: 100, from: alice, gas: 4000000 })

      // from txB extension above
      await plasma.methods
        .beginExit(0, 5, end, end + 100)
        .send({ value: 0, from: alice, gas: 4000000 })
      let exitID = exitNonce
      exitNonce++
      ...
      await plasma.methods
        .respondInvalidHistoryDeposit(chalID, depositEnd)
        .send()
    ...
    it('should allow challengeInvalidHistoryWithDeposit s and responses', async () => {
      ...
      const coinID = 0 // could be any in deposit so [0,99]
      await plasma.methods
        .challengeInvalidHistoryWithDeposit(exitID, coinID, end)
        .send({ value: 0, from: alice, gas: 4000000 })
      let chalID = chalNonce
      chalNonce++
