...
describe('ERC20 Token Support', () => {
  ...
  let bytecode,
    ...
    freshTokenSnapshot,
    serializer // eslint-disable-line no-unused-vars
  ...
  describe('Non-ETH Listings, Deposits, and Exits', () => {
    ...
    it('should allow anyone to list a new token', async () => {
      ...
      const listingAddress = await plasma.methods
        .listings__contractAddress(listingNonce)
        .call()
      const listingDenomination = await plasma.methods
        .listings__decimalOffset(listingNonce)
        .call()
    ...
    const aliceNumPlasmaCoins =
      aliceDepositSize * Math.pow(10, benCoinDenomination)
    it('should allow allice to approve and deposit', async () => {
      await token.methods
        .approve(plasma._address, aliceDepositSize)
        .send({ from: alice })

      await plasma.methods
        .depositERC20(token._address, aliceDepositSize)
        .send({ from: alice })

      const newContractBalance = await token.methods
        .balanceOf(plasma._address)
        .call()
      expect(newContractBalance).to.equal(aliceDepositSize)

      const newDepositStart = await plasma.methods
        .deposits__untypedStart(1, aliceNumPlasmaCoins)
        .call()
      expect(newDepositStart).to.equal('0') // first deposit tokentype 1 ^ should equal 0 since it was the first

      const newDepositer = await plasma.methods
        .deposits__depositer(1, aliceNumPlasmaCoins)
        .call()
      expect(newDepositer).to.equal(alice)
    })
    it('should allow bob to exit the ERC20s if uncontested', async () => {
      await plasma.methods
        .beginExit(benTokenType, 0, 0, aliceNumPlasmaCoins)
        .send({ from: bob })
      ...
      const newPlasmaBalance = await token.methods
        .balanceOf(plasma._address)
        .call()
  ...
  describe('Exit Games for non-ETH tokens', () => {
    ...
    otherTXs.forEach((tx) => {
      tx.transfers[0].token = new BigNum(1)
    })
    const blocks = {
    ...
    before(async function() {
      ...
      await plasma.methods
        .depositERC20(token._address, end)
        .send({ from: alice, gas: 4000000 })
    ...
    it('should allow inclusionChallenges and respondTransactionInclusion', async () => {
      await plasma.methods
        .beginExit(tokenType, 5, start, end)
    ...
    it('should allow respondDepositInclusion s', async () => {
      await plasma.methods
        .beginExit(tokenType, 2, start, end)
        .send({ value: 0, from: alice, gas: 4000000 })
    ...
    it('should allow Spent Coin Challenges to cancel exits', async () => {
      ...
      await plasma.methods
        .beginExit(tokenType, 3, start, end)
    ...
    it('should allow challengeBeforeDeposit s', async () => {
      await plasma.methods
        .beginExit(tokenType, 0, start, end)
        .send({ value: 0, from: alice, gas: 4000000 })
      const exitID = exitNonce
      exitNonce++
    ...
    it('should allow challengeInvalidHistoryWithTransaction s and respondInvalidHistoryTransaction s', async () => {
      await plasma.methods
        .beginExit(tokenType, 4, start, end)
        .send({ value: 0, from: dave, gas: 4000000 })
      let exitID = exitNonce
      exitNonce++
    ...
    it('should allow respondInvalidHistoryDeposit s', async () => {
      ...
      await token.methods.approve(plasma._address, end).send({ from: alice })
      await plasma.methods
        .depositERC20(token._address, 100)
        .send({ from: alice, gas: 4000000 })

      // from txB extension above
      await plasma.methods
        .beginExit(tokenType, 5, end, end + 100)
        .send({ value: 0, from: alice, gas: 4000000 })
      let exitID = exitNonce
      exitNonce++
