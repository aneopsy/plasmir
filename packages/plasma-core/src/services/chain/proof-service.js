  ...
  async _transactionValid(transaction, proof) {
    ...
    if (root === EMPTY_BLOCK_HASH) {
      if (transaction.transfers.length > 0) {
        this.log(
          `WARNING: Block #${transaction.block} is empty but received a non-empty proof element. Proof will likely be rejected. This is probably due to an error in the operator.`
        )
      }
