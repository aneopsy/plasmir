...
class SyncService extends BaseService {
  ...
  async _checkPendingTransactions() {
    ...
    for (let i = 0; i < this.pending.length; i++) {
      ...
      try {
        ...
        this.log(`ERROR: ${err}`)
        this.log(
          `Ran into an error while importing transaction: ${tx.hash}, trying again in a few seconds...`
        )
      }
    }
  ...
  async _addTransaction(tx) {
    ...
    try {
      ...
      this.log(
        `ERROR: Operator failed to return information for transaction: ${tx.hash}`
      )
      throw err
    }
