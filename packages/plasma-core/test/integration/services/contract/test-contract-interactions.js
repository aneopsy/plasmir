...
const compiledContracts = require('@plasmir/plasma-contracts')
...
const deployPlasmaChain = async (web3, operator, name, ip) => {
  ...
  const serializer = await deployContract(
    web3,
    operator,
    compiledContracts.serializerCompiled
  )
  const template = await deployContract(
    web3,
    operator,
    compiledContracts.plasmaChainCompiled
  )
  const registry = await deployContract(
    web3,
    operator,
    compiledContracts.plasmaRegistryCompiled
  )

  await registry.methods
    .initializeRegistry(template.options.address, serializer.options.address)
    .send({
      from: operator,
      gas: 7000000,
    })
  const receipt = await registry.methods
    .createPlasmaChain(operator, name, ip)
    .send({
      from: operator,
      gas: 7000000,
    })
  const plasmaChainAddress =
    receipt.events.NewPlasmaChain.returnValues['PlasmaChainAddress']
...
describe('Contract Interactions', () => {
  ...
  before(async () => {
    ...
    const { registryAddress } = await deployPlasmaChain(
      web3,
      operator,
      'testchain',
      '0.0.0.0'
