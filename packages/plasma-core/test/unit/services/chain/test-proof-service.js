...
const utils = require('@plasmir/plasma-utils')
...
const submitDeposits = async (deposits) => {
  for (let deposit of deposits) {
    await app.services.contract.deposit(
      0,
...
const sign = (address, transaction) => {
  ...
  return web3.eth.accounts
    .privateKeyToAccount(account.key)
    .sign('0x' + hash(transaction)).signature
}
...
describe('ProofService', async () => {
  ...
  it('should correctly check a valid transaction proof with many deposits', async () => {
    ...
    const validity = await verifier.checkProof(
      manyDepositTx2,
      manyDeposits,
      manyDepositProof
    )
  ...
  it('should not verify an invalid set of deposits', async () => {
    ...
    await verifier
      .checkProof(transaction2, invalidDeposits, proof)
      .should.be.rejectedWith('Invalid deposit')
  })
  ...
  it('should not verify an invalid range', async () => {
    ...
    await verifier
      .checkProof(invalidTransaction, deposits, proof)
      .should.be.rejectedWith('Invalid state transition')
  })
  ...
  it('should not verify with missing chunks of history', async () => {
    ...
    invalidProof[0].proof[0].signature = sign(
      invalidProof[0].transaction.transfers[0].sender,
      invalidProof[0].transaction
    )
    await app.services.chaindb.addBlockHeader(
      new BigNum(1),
      hash(invalidProof[0].transaction)
    )

    await verifier
      .checkProof(transaction2, deposits, invalidProof)
      .should.be.rejectedWith('Invalid state transition')
  })
  ...
  it('should not verify a transaction with an invalid signature', async () => {
    let invalidProof = _.cloneDeep(proof)
    invalidProof[0].proof[0].signature = sign(
      invalidProof[0].transaction.transfers[0].recipient,
      invalidProof[0].transaction
    )
