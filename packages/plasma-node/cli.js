...
const Plasma = require('@plasmir/plasma-js-lib')
...
program
  ...
  .option(
    '-h, --hostname <hostname>',
    'Host the node is running on. Defaults to 127.0.0.1.',
    '127.0.0.1'
  )
  .option(
    '-p, --port <port>',
    'Port the node is running on. Defaults to 3000.',
    '3000'
  )

const client = new Plasma(
  new Plasma.providers.HttpProvider(
    `http://${program.hostname}:3000`
    // `http://${program.hostname}:${program.port}`
  )
)
...
program
  ...
  .action(async (account, token, amount) => {
    ...
    console.log(
      `View deposit on Etherscan: https://rinkeby.etherscan.io/tx/${result.transactionHash}`
    )
  })
...
program.command('finalizeexits <account>').action(async (account) => {
  account = await parseAccount(account)
...
program.command('submitblock').action(async () => {
