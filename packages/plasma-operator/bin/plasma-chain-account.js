...
const colors = require('bin/node_modules/colors') // eslint-disable-line no-unused-vars
const inquirer = require('bin/node_modules/inquirer')
const appRoot = require('../src/utils.js').appRoot
const Web3 = require('bin/node_modules/web3')
...
program
  ...
  .action(async (none, cmd) => {
    ...
    if (accounts.length) {
      console.log(
        'Account already created! Try starting the operator with `operator start`'
      )
      return
    }
    ...
    const keystorePath = path.join(
      keystoreDirectory,
      new Date().toISOString() + '--' + newAccount.address
    )
