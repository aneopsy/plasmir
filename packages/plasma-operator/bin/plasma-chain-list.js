...
program
  ...
  .action(async (none, cmd) => {
    ...
    const plasmaRegistryCt = new web3.eth.Contract(
      plasmaRegistryCompiled.abi,
      config.plasmaRegistryAddress
    )
    plasmaRegistryCt.getPastEvents(
      'NewPlasmaChain',
      {
        fromBlock: 0,
        toBlock: 'latest',
      ...
      (err, res) => {
        ...
        console.log(
          '~~~~~~~~~~~~~~~~~~~'.rainbow,
          '\nList of Plasma Chains Deployed to the registry:'.white.bold,
          config.plasmaRegistryAddress.white.bold
        )
        for (const ethEvent of res) {
          ...
          const name = Buffer.from(
            ethEvent.returnValues['2'].slice(2),
            'hex'
          ).toString('utf8')
          const ip = Buffer.from(
            ethEvent.returnValues['3'].slice(2),
            'hex'
          ).toString('utf8')
          console.log(`
${'~~~~~~~~~~~~~~~~~~~'.rainbow}
