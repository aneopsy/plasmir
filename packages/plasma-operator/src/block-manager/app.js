...
const SignedTransaction = require('@plasmir/plasma-utils').serialization.models
  .SignedTransaction
    ...
    async (eventType, filename) => {
      ...
      try {
        ...
        process.send({
          ipcID: -1,
          message: {
            rootHash: Buffer.from(newBlock.rootHash, 'hex').toString('hex'),
          },
        })
      } catch (err) {
...
process.on('message', async (m) => {
  ...
  if (m.message.method === constants.INIT_METHOD) {
    ...
    try {
      ...
      const depositTx = getDepositTransaction(
        m.message.params.recipient,
        new BN(m.message.params.token, 16),
        new BN(m.message.params.start, 16),
        new BN(m.message.params.end, 16)
      )
      addDepositRes = await blockStore.addDeposit(depositTx)
    } catch (err) {
      error(
        'Error in adding deposit!\nrpcID:',
        m.message.id,
        '\nError message:',
        err.toString(),
        '\n'
    ...
    try {
      ...
      console.error(
        'Error in getting history proof!\nrpcID:',
        m.message.id,
        '\nError message:',
        err.toString(),
        '\n'
    ...
    let response
    try {
      const txs = await blockStore.getTransactions(
        blockNum,
        blockNum,
        token,
        start,
  ...
  process.send({
    ipcID: m.ipcID,
    message: { error: 'RPC method not recognized!' },
  })
