...
async function startup(config) {
  ...
  es.plasmaChain = new es.web3.eth.Contract(
    plasmaChainCompiled.abi,
    es.ethDB.plasmaChainAddress,
    { from: es.operatorAddress }
  )
...
function _getEventWatchers(config) {
  ...
  for (const ethEvent of Object.keys(es.plasmaChain.events)) {
    ...
    eventWatchers[ethEvent] = new EventWatcher(
      es.web3,
      es.plasmaChain,
      ethEvent,
      config.finalityDepth,
      config.ethPollInterval
    )
  }
...
async function deployNewPlasmaRegistry(config) {
  ...
  const plasmaSerializerCt = new es.web3.eth.Contract(
    serializerCompiled.abi,
    es.operatorAddress,
    { from: es.operatorAddress, gas: 7000000, gasPrice: '5000000000' }
  )
  const plasmaChainCt = new es.web3.eth.Contract(
    plasmaChainCompiled.abi,
    es.operatorAddress,
    { from: es.operatorAddress, gas: 7000000, gasPrice: '5000000000' }
  )
  const plasmaRegistryCt = new es.web3.eth.Contract(
    plasmaRegistryCompiled.abi,
    es.operatorAddress,
    { from: es.operatorAddress, gas: 7000000, gasPrice: '5000000000' }
  )
  const serializer = await plasmaSerializerCt
    .deploy({ data: serializerCompiled.bytecode })
    .send()
  // To set up the Plasma Network, we need to first deploy a Plasma Chain contract
  const plasmaChain = await plasmaChainCt
    .deploy({ data: plasmaChainCompiled.bytecode })
    .send()
  // Finally deploy the Plasma Registry and save the address in our ethDB
  const plasmaRegistry = await plasmaRegistryCt
    .deploy({ data: plasmaRegistryCompiled.bytecode })
    .send()
  ...
  await plasmaRegistry.methods
    .initializeRegistry(plasmaChain.options.address, serializer.options.address)
    .send()
}
...
async function initializeProdEnv(config) {
  ...
  if (es.ethDB.plasmaRegistryAddress === DEPLOY_REGISTRY) {
    ...
    if (
      config.web3HttpProvider !== undefined &&
      config.web3HttpProvider.includes('rinkeby')
    ) {
      console.log(
        'View transaction progress on Etherscan:',
        'https://rinkeby.etherscan.io/address/'.blue + es.operatorAddress.blue
      )
    }
    await deployNewPlasmaRegistry(config)
  ...
  if (es.ethDB.plasmaChainAddress === undefined) {
    ...
    const plasmaRegistryCode = await es.web3.eth.getCode(
      es.ethDB.plasmaRegistryAddress
    )
    if (plasmaRegistryCode === '0x') {
      throw new Error(
        'No plasma registry found at address: ' + es.ethDB.plasmaRegistryAddress
      )
    }
    ...
    console.log(
      'Plasma Chain contract already deployed. Skipping deployment...'.green
    )
  }
}
...
async function deployNewPlasmaChain(web3, config) {
  // We have the compiled contracts, let's create objects for them...
  const plasmaRegistry = new web3.eth.Contract(
    plasmaRegistryCompiled.abi,
    es.ethDB.plasmaRegistryAddress
  )
  ...
  try {
    ...
    createPChainReciept = await plasmaRegistry.methods
      .createPlasmaChain(
        es.operatorAddress,
        Buffer.from(config.plasmaChainName),
        Buffer.from(config.operatorIpAddress)
      )
      .send({ from: es.operatorAddress, gas: 800000, gasPrice: '5000000000' })
  } catch (err) {
  ...
  const newPlasmaChainAddress =
    createPChainReciept.events.NewPlasmaChain.returnValues['0']
...
async function submitRootHash(rootHash) {
  ...
  while (currentRootNum !== rootNum) {
    // eslint-disable-line no-unmodified-loop-condition
    log(
      'Multiple roots attempting to be submitted at the same time. Waiting until other submit root has finished'
    )
    await timeout(100)
  }
  const reciept = await es.plasmaChain.methods
    .submitBlock('0x' + rootHash)
    .send({ gas: 400000, gasPrice: '5000000000' })
  currentRootNum++
  log(
    'Ethereum reciept for block number:',
    reciept.events.SubmitBlockEvent.returnValues['0'],
    'wish root hash:',
    reciept.events.SubmitBlockEvent.returnValues['1']
  )
}
