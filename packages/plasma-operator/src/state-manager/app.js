...
process.on('message', async (m) => {
  ...
  if (m.message.method === constants.INIT_METHOD) {
    ...
    process.send({
      ipcID: m.ipcID,
      message: { result: blockNumber.toString() },
    })
    return
    // ******* DEPOSIT ******* //
    ...
    try {
      ...
      const getTxResult = Array.from(
        await state.getTransactions(address, startBlock, endBlock)
      )
      response = { result: getTxResult }
    } catch (err) {
      error(
        'Error in getting past transactions!\nrpcID:',
        m.message.id,
        '\nError message:',
        err,
        '\n'
