...
const BLOCKNUMBER_BYTE_SIZE = require('../../src/constants.js')
  .BLOCKNUMBER_BYTE_SIZE
// const constants = require('../../src/constants.js')
const models = require('@plasmir/plasma-utils').serialization.models
const PlasmaMerkleSumTree = require('@plasmir/plasma-utils').PlasmaMerkleSumTree
const UnsignedTransaction = models.UnsignedTransaction
const TransferProof = models.TransferProof
const getDepositTransaction = require('../../src/utils.js')
...
describe('BlockStore', function() {
  ...
  beforeEach(async () => {
    ...
    fs.copyFileSync(
      path.join(__dirname, 'tx-log', TEST_BLOCK_FILENAME),
      config.txLogDir + '0001'
    )
  ...
  it.skip('gets transaction leaves over a number of blocks correctly', async () => {
    ...
    for (let i = 1; i < 4; i++) {
      ...
      const txBundle = getTxBundle(TXs)
      const blockNumber = new BN(i).toArrayLike(
        Buffer,
        'big',
        BLOCKNUMBER_BYTE_SIZE
      )
    ...
    // begin test
    const rangeSinceBlockZero = await blockStore.getTransactions(
      new BN(1),
      blockStore.blockNumberBN,
      new BN(0),
      new BN(1),
      new BN(2)
    )
  ...
  it('generates history proofs correctly', async () => {
    ...
    for (let i = 1; i < 3; i++) {
      ...
      const reciept = await plasmaChain.methods
        .submitBlock(rootHash)
        .send({ gas: 400000 })
      expect(reciept.events.SubmitBlockEvent.returnValues['0']).to.equal(
        blockStore.blockNumberBN.toString()
      )
      expect(reciept.events.SubmitBlockEvent.returnValues['1']).to.equal(
    ...
    const txsAndProofs = await blockStore.getTxsWithProofs(
      new BN(1),
      new BN(1),
      new BN(0),
      start,
  ...
  it.skip('generates history proofs correctly when given a particular transaction', async () => {
    ...
    for (let i = 0; i < depositTxs.length; i++) {
      ...
      depositTxs[i] = getDepositTransaction(
        tr.recipient,
        tr.token,
        tr.start,
        tr.end,
    ...
    for (let i = 1; i < 100; i++) {
      ...
      const plasmaUtilsTree = new PlasmaMerkleSumTree(
        txBundle.map((value) => value[0])
      )
      expect(plasmaUtilsTree.root().hash).to.equal(
    ...
    // Get the tx proofs for the range
    const txsAndProofs = await blockStore.getTxHistory(
      new BN(1),
      new BN(99),
      proofTx
    )
    await verifyTxsAndProofs(
      txsAndProofs.transactionHistory,
      web3,
      plasmaChain,
      roots
    )
  ...
  for (const blockNumber of Object.keys(txsAndProofs)) {
    ...
    for (const txAndProof of txsAndProofs[blockNumber]) {
      ...
      for (const [
        i,
        trProofDecoded,
      ] of txAndProof.transactionProof.transferProofs.entries()) {
        // Actually check inclusion. LOL
        trProofDecoded.inclusionProof = getHexStringProof(
          trProofDecoded.inclusionProof
        )
        ...
        const res = await plasmaChain.methods
          .checkTransferProofAndGetBounds(
        ...
        const result = PlasmaMerkleSumTree.checkTransferProof(
          unsignedTx,
          i,
          transferProof,
          roots[0].slice(2) + 'ffffffffffffffffffffffffffffffff'
        )
